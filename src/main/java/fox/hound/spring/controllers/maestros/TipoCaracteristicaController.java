...
@RestController
@RequestMapping("tipoCaracteristica")
public class TipoCaracteristicaController {

	@Autowired
	private MaestroService service;
	
	private Class<?> CLASE = TipoCaracteristica.class;

	@RequestMapping(value="/buscarTodos", method=RequestMethod.GET, produces=MediaType.APPLICATION_JSON_UTF8_VALUE)
	public ResponseEntity<?> getAll(HttpServletRequest request) {
		return ResponseDefault.ok(service.getAll("TipoCaracteristica"), CLASE, ResponseDefault.PLURAL);
	}
	...
	public ResponseEntity<?> borrar(@PathVariable String id, HttpServletRequest request) {
		...
		return ResponseDefault.message(MessageUtil.ELIMINAR_REGISTRO, "Tipo de Caracteristica");
	}
