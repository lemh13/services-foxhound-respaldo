...
public class TipoServicioController {
	...
	@RequestMapping(value="categoria/{categoriaId}/agregar", method=RequestMethod.POST, produces=MediaType.APPLICATION_JSON_UTF8_VALUE)
	public ResponseEntity<?> agregar(@RequestBody TipoServicio clase, @PathVariable String categoriaId, HttpServletRequest request) {
		...
		if (categoria != null) {
			...
			return ResponseDefault.messageAndObject(MessageUtil.GUARDAR_REGISTRO, "Tipo de Servicio", service.saveOrUpdate(clase), CLASE, ResponseDefault.SINGULAR);			
		} else {
			return ResponseDefault.message(MessageUtil.ERROR_ASOCIACION, "Categoria");
		}	
	}

	@RequestMapping(value="categoria/{categoriaId}/modificar", method=RequestMethod.PUT, produces=MediaType.APPLICATION_JSON_UTF8_VALUE)
	public ResponseEntity<?> modificar(@RequestBody TipoServicio clase, @PathVariable String categoriaId, HttpServletRequest request) {
	
		Categoria categoria = categoriaService.getOne(Long.valueOf(categoriaId));
		
		if (categoria != null) {
			clase.setCategoria(categoria);
			return ResponseDefault.messageAndObject(MessageUtil.ACTUALIZAR_REGISTRO, "Tipo de Servicio", service.saveOrUpdate(clase), CLASE, ResponseDefault.SINGULAR);			
		} else {
			return ResponseDefault.message(MessageUtil.ERROR_ASOCIACION, "Categoria");
		}
	}
	...
	public ResponseEntity<?> borrar(@PathVariable String id, HttpServletRequest request) {
		...
		return ResponseDefault.message(MessageUtil.ELIMINAR_REGISTRO, "Tipo de Servicio");
	}
